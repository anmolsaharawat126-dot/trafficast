...
# --------------------------------------------------------------------------- #
#  Author credit — floating "Created by Anmol Saharawat" popup + GitHub link
# --------------------------------------------------------------------------- #
GITHUB_URL = "https://github.com/anmolsaharawat126-dot"
AUTHOR = "Anmol Saharawat"
_GH_SVG = ('<svg height="17" width="17" viewBox="0 0 16 16" fill="currentColor" '
           'style="vertical-align:middle"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 '
...
st.markdown(f"""<style>
.tc-fab {{ position:fixed; right:18px; bottom:18px; z-index:99999; display:inline-flex;
  align-items:center; gap:10px; text-decoration:none; padding:10px 15px; border-radius:14px;
  font-family:'Outfit',sans-serif; font-weight:700; font-size:13.5px; color:#fff;
  background:linear-gradient(135deg,#3b82f6,#60a5fa); box-shadow:0 10px 30px rgba(59,130,246,.45);
  border:1px solid rgba(255,255,255,.25); transition:.18s; cursor:pointer; animation:tcPop .5s ease-out; }}
.tc-fab:hover {{ transform:translateY(-3px) scale(1.03); filter:brightness(1.07);
  box-shadow:0 14px 38px rgba(59,130,246,.6); }}
.tc-fab .gh {{ display:flex; color:#fff; }}
.tc-fab .nm small {{ display:block; font-weight:600; font-size:10px; opacity:.85;
...
<a class="tc-fab" href="{GITHUB_URL}" target="_blank" rel="noopener"
   title="Created by {AUTHOR} · open GitHub profile">
   <span class="gh">{_GH_SVG}</span>
   <span class="nm">Created by {AUTHOR}<small>github · TraffiCast AI</small></span>
</a>""", unsafe_allow_html=True)
if not st.session_state.get("_welcomed"):
    try:
        st.toast(f"🚦 TraffiCast AI — Created by {AUTHOR}", icon="✨")
    except Exception:
        pass
...
.tc-credit {{ margin-top:14px; padding-top:12px; border-top:1px solid rgba(148,163,184,.25); }}
.tc-credit a {{ display:inline-flex; align-items:center; gap:9px; text-decoration:none;
  color:#94a3b8; font-size:13px; font-weight:700; font-family:'Outfit',sans-serif; padding:8px 12px;
  border-radius:11px; border:1px solid rgba(148,163,184,.25); background:rgba(255,255,255,.03);
  transition:.18s; }}
.tc-credit a:hover {{ color:#fff; border-color:#3b82f6;
  background:linear-gradient(135deg,rgba(59,130,246,.18),rgba(96,165,250,.10)); }}
.tc-credit .lbl small {{ display:block; color:#94a3b8; font-weight:500; font-size:10px;
  text-transform:uppercase; letter-spacing:.05em; }}
</style>
<div class="tc-credit">
  <a href="{GITHUB_URL}" target="_blank" rel="noopener" title="Created by {AUTHOR} · open GitHub">
     <span class="gh" style="color:#e7ecf5;display:flex">{_GH_SVG}</span>
     <span class="lbl">Created by {AUTHOR}<small>github · click to open</small></span>
  </a>
...
# =========================================================================== #
    ...
    if routes:
        ...
        fig.add_trace(go.Scattermapbox(
            ...
            mode="lines", line=dict(width=3, color="gray"), name="Bypass Route 2"
        ))
